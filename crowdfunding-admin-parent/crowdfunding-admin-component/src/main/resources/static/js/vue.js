...
(function (global, factory) {
  ...
  var isAndroid = (UA && UA.indexOf('android') > 0) || (weexPlatform
      === 'android');
  var isIOS = (UA && /iphone|ipad|ipod|ios/.test(UA)) || (weexPlatform
      === 'ios');
  ...
  var isServerRendering = function () {
    if (_isServer === undefined) {
      ...
      if (!inBrowser && !inWeex && typeof global !== 'undefined') {
        ...
        _isServer = global['process'] && global['process'].env.VUE_ENV
            === 'server';
      } else {
        _isServer = false;
      }
  ...
  function validateComponentName(name) {
    if (!new RegExp(
        ("^[a-zA-Z][\\-\\.0-9_" + (unicodeRegExp.source) + "]*$")).test(name)) {
      warn(
    ...
    var hasHandler = {
      has: function has(target, key) {
        ...
        var isAllowed = allowedGlobals(key) ||
            (typeof key === 'string' && key.charAt(0) === '_' && !(key
                in target.$data));
        if (!has && !isAllowed) {
    ...
    function invoker() {
      ...
      if (Array.isArray(fns)) {
        ...
        for (var i = 0; i < cloned.length; i++) {
          invokeWithErrorHandling(cloned[i], null, arguments$1, vm,
              "v-on handler");
        }
      } else {
    ...
    if (vnode instanceof VNode) {
      ...
      for (var i = 0; i < vnodes.length; i++) {
        res[i] = cloneAndMarkFunctionalResult(vnodes[i], data,
            renderContext.parent, options, renderContext);
      }
      return res
    }
  }
  ...
  function initRender(vm) {
    ...
    /* istanbul ignore else */
    {
      defineReactive$$1(vm, '$attrs',
          parentData && parentData.attrs || emptyObject, function () {
            !isUpdatingChildComponent && warn("$attrs is readonly.", vm);
          }, true);
      defineReactive$$1(vm, '$listeners',
  ...
  function initMethods(vm, methods) {
    ...
    for (var key in methods) {
      ...
      vm[key] = typeof methods[key] !== 'function' ? noop : bind(methods[key],
          vm);
    }
  }
      ...
      if (isDef(tag)) {
        if (isDef(children)) {
          ...
          if (!elm.hasChildNodes()) {
            ...
            if (isDef(i = data) && isDef(i = i.domProps) && isDef(
                i = i.innerHTML)) {
              if (i !== elm.innerHTML) {
                /* istanbul ignore if */
  ...
  function getRawDirName(dir) {
    return dir.rawName || ((dir.name) + "." + (Object.keys(
        dir.modifiers || {}).join('.')))
  }
  ...
  function genCheckboxModel(
    ...
    addHandler(el, 'change',
        ...
        +
        '$$i=_i($$a,$$v);' +
        "if($$el.checked){$$i<0&&(" + (genAssignmentCode(value,
            '$$a.concat([$$v])')) + ")}" +
  ...
  function genRadioModel(
    ...
    addHandler(el, 'change', genAssignmentCode(value, valueBinding), null,
        true);
  }
  ...
  function updateDOMListeners(oldVnode, vnode) {
    ...
    target$1 = vnode.elm;
    normalizeEvents(on);
    updateListeners(on, oldOn, add$1, remove$2, createOnceHandler$1,
        vnode.context);
    target$1 = undefined;
  }
  ...
  function updateDOMProps(oldVnode, vnode) {
    ...
    for (key in props) {
      ...
      } else if (key === 'innerHTML' && isSVG(elm.tagName) && isUndef(
          elm.innerHTML)) {
  ...
  function getStyle(vnode, checkChild) {
    ...
    while ((parentNode = parentNode.parent)) {
      if (parentNode.data && (styleData = normalizeStyleData(
          parentNode.data))) {
        extend(res, styleData);
      }
    }
  ...
  var setProp = function (el, name, val) {
    ...
    if (cssVarRE.test(name)) {
      ...
      el.style.setProperty(hyphenate(name), val.replace(importantRE, ''),
          'important');
    } else {
      ...
      if (vnode.tag === 'select') {
        ...
        if (curOptions.some(function (o, i) {
          ...
          var needReset = el.multiple
              ...
              : binding.value !== binding.oldValue && hasNoMatchingOption(
              binding.value, curOptions);
          if (needReset) {
            trigger(el, 'change');
          }
        }
          ...
          if (comment.test(html)) {
            ...
            if (commentEnd >= 0) {
              if (options.shouldKeepComment) {
                options.comment(html.substring(4, commentEnd), index,
                    index + commentEnd + 3);
              }
    ...
    function parseStartTag() {
      ...
      if (start) {
        ...
        while (!(end = html.match(startTagClose)) && (attr = html.match(
            dynamicArgAttribute) || html.match(attribute))) {
          attr.start = index;
    ...
    if (exp) {
      {
        ...
        if (el.for) {
          ...
          if (iterator && iterator === exp && parent && parent.tag
              === 'transition-group') {
            warn$2(
  ...
  function processSlotContent(el) {
    ...
    if (slotTarget) {
      ...
      el.slotTargetDynamic = !!(el.attrsMap[':slot']
    ...
    // 2.6 v-slot syntax
    {
      if (el.tag === 'template') {
        ...
        if (slotBinding$1) {
          ...
          var slotContainer = slots[name$1] = createASTElement('template', [],
              el);
      ...
      if (dirRE.test(name)) {
        ...
        if (bindRE.test(name)) { // v-bind
          ...
          if ((modifiers && modifiers.prop) || (
              !el.component && platformMustUseProp(el.tag, el.attrsMap.type,
                  name)
          )) {
  ...
  function genHandlers(
    ...
    if (dynamicHandlers) {
      return prefix + "_d(" + staticHandlers + ",[" + (dynamicHandlers.slice(0,
          -1)) + "])"
    } else {
  ...
  function genHandler(handler) {
    ...
    if (!handler.modifiers) {
      ...
      return ("function($event){" + (isFunctionInvocation ? ("return "
          + (handler.value)) : handler.value) + "}") // inline statement
    } else {
      var code = '';
      var genModifierCode = '';
      var keys = [];
      ...
      if (needRuntime) {
        ...
        res += "{name:\"" + (dir.name) + "\",rawName:\"" + (dir.rawName) + "\""
            + (dir.value ? (",value:(" + (dir.value) + "),expression:"
                + (JSON.stringify(dir.value))) : '') + (dir.arg ? (",arg:"
                    + (dir.isDynamicArg ? dir.arg : ("\"" + (dir.arg) + "\"")))
                : '') + (dir.modifiers ? (",modifiers:" + (JSON.stringify(
                dir.modifiers))) : '') + "},";
      }
    }
    if (hasRuntime) {
  ...
  function genScopedSlots(
    ...
    return ("scopedSlots:_u([" + generatedSlots + "]" + (needsForceUpdate
        ? ",null,true" : "") + (!needsForceUpdate && needsKey ? (",null,false,"
        + (hash(generatedSlots))) : "") + ")")
  }
        ...
        if (dirRE.test(name)) {
          ...
          if (value) {
            ...
            if (name === 'v-for') {
              ...
              checkFunctionParameterExpression(value,
                  (name + "=\"" + value + "\""), warn, range);
            } else if (onRE.test(name)) {
              checkEvent(value, (name + "=\"" + value + "\""), warn, range);
            } else {
              checkExpression(value, (name + "=\"" + value + "\""), warn,
                  range);
            }
          }
        }
  ...
  function checkIdentifier(
    ...
    if (typeof ident === 'string') {
      try {
        ...
        warn(("invalid " + type + " \"" + ident + "\" in expression: "
            + (text.trim())), range);
      }
    }
  }
