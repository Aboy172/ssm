...
function fillAuthTree() {
  ...
  if (ajaxReturn.responseJSON.code !== 200) {
    layer.msg(" 请 求 处 理 出 错 ！ 响 应 状 态 码 是 ： " + ajaxReturn.responseJSON.code
        + " 说 明 是:+  " + ajaxReturn.responseJSON.message);
    return;
  }
...
function fillTableBody(page) {
  ...
  $("#rolePageBody").empty();
  $("#Pagination").empty();
  if (page == null || page == undefined || page.records == null
  ...
  for (var i = 0; i < page.records.length; i++) {
    ...
    var buttonTd = "<td>" + checkBtn + " " + pencilBtn + " " + removeBtn
        + "</td>";
    var tr = "<tr>" + roleTd + numberTd + checkboxTd + roleNameTd + buttonTd
        + "</tr>";
